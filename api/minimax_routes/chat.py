...
def _build_chat_kwargs(
    ...
    """Build kwargs dict for OpenAI-compatible chat API call."""
    kwargs: Dict[str, Any] = {
...
async def create_chat_completion(
    ...
    """Creates a chat completion via MiniMax Cloud API."""
    ...
    logger.debug(f"==== MiniMax chat request ====\n{kwargs}")

    if request.stream:
        def _sync_stream() -> Iterator:
            stream = engine.client.chat.completions.create(**kwargs)
            for chunk in stream:
                yield chunk
        ...
        response = await run_in_threadpool(
            engine.client.chat.completions.create, **kwargs
        )
        return response
